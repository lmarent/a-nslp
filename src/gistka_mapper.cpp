...
/// Utilities for mapping between GISTka/protlib and A-NSLP.
/// ----------------------------------------------------------
/// $Id: gistka_mapper.cpp 3093 2015-08-31 10:58:00Z amarentes $
...
ntlp::APIMsg *gistka_mapper::create_api_msg(msg::ntlp_msg *msg) const throw () 
{
	 ...
	 */
	const anslp_msg *m = msg->get_anslp_msg();
	...
	uint32 nslp_id = anslp_config::NSLP_ID;
	...
	uint32 timeout = anslp_config::SEND_MSG_TIMEOUT;
	uint16 ip_ttl = anslp_config::SEND_MSG_IP_TTL;
	uint32 ntlp_hop_count = anslp_config::SEND_MSG_GIST_HOP_COUNT;
	...
	apimsg->set_source(anslp_config::INPUT_QUEUE_ADDRESS);
 ...
 *
 * This is called each time the NTLP gets a A-NSLP message from the network.
 */
		...
		const ntlp::APIMsg *apimsg) const 
{
	...
	session_id *sid = create_anslp_session_id(apimsg->get_sessionid());
	...
	IE *ie = mgr->deserialize(payload, cat_anslp_msg, IE::protocol_v1,
		errlist, num_read, false);

	if ( ie == NULL ) {
		assert( ! errlist.is_empty() );
		LogError("deserializing ANSLP message failed: "
				<< *(errlist.get()) );
		LogDebug("nslp-data (the ANSLP message) payload: " << payload);
	}

	/*
	 * With the deserialize() call above, the ANSLP_IEManager can only
	 * create subclasses of anslp_msg. If it doesn't something has gone
	 * wrong with the IE registration, which would be a programming error.
	 */
	anslp_msg *m = NULL;

	if ( ie != NULL ) {
		m = dynamic_cast<anslp_msg *>(ie);
		assert( m != NULL );
	}
	...
	switch ( status ) {
	  case APIMsg::route_changed_status_bad:
		return new route_changed_bad_event(
			create_anslp_session_id(msg->get_sessionid()) );
		break;
...
event *gistka_mapper::map_timer_message(const anslp_timer_msg *msg) const 
{
	assert( msg != NULL );
 ...
 */
session_id *gistka_mapper::create_anslp_session_id(ntlp::sessionid *sid) const 
{
	assert( sid != NULL );
