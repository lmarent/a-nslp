/// ----------------------------------------*- mode: C++; -*--
/// @file anslp_notify.cpp
/// Implementation of the ANSLP NOTIFY Message.
/// ----------------------------------------------------------
/// $Id: anslp_notify.cpp 2558 2015-08-30 $
/// $HeadURL: https://./src/msg/anslp_notify.cpp $
// ===========================================================
//                      
 ...
 */
anslp_notify::anslp_notify() : anslp_msg(MSG_TYPE) 
{
	// nothing to do
}
 ...
 */
anslp_notify::anslp_notify(const anslp_notify &other)
		: anslp_msg(other) 
{
 ...
 */
anslp_notify::~anslp_notify() 
{
	// Nothing to do, parent class handles this.
}


anslp_notify *anslp_notify::new_instance() const 
{
	anslp_notify *inst = NULL;
	catch_bad_alloc(inst = new anslp_notify());
	return inst;
}


anslp_notify *anslp_notify::copy() const 
{
	anslp_notify *copy = NULL;
	catch_bad_alloc(copy = new anslp_notify(*this));
	return copy;
}


void anslp_notify::serialize(NetMsg &msg, coding_t coding,
...
uint32 
anslp_notify::serialize_object(ie_object_key &key, NetMsg &msg, coding_t coding) const
{
...
bool anslp_notify::check() const {
	
	// Verifies that information code exists.
	ie_object_key key_inf(information_code::OBJECT_TYPE, 1);
	anslp_object *obj= get_object(key_inf);
	if (obj == NULL)
		return false;
...
void anslp_notify::register_ie(IEManager *iem) const {
 ...
 */
void anslp_notify::set_information_code(uint8 severity, uint8 response_code,
                uint16 object_type) {
 ...
 *
 * @return a ANSLP object type (16 bit)
 */
uint16 anslp_notify::get_object_type() const 
{
 ...
 */
uint8 anslp_notify::get_severity_class() const 
{
 ...
 */
uint8 anslp_notify::get_response_code() const 
{
 ...
 */
uint16 
anslp_notify::get_response_object_type() const 
{
