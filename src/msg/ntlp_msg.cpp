...
/// An NTLP Message.
/// ----------------------------------------------------------
/// $Id: ntlp_msg.cpp 2558 2015-08-30 15:27:00 amarentes $
 ...
 * Constructor.
 */
ntlp_msg::ntlp_msg(const session_id &id, anslp_msg *body,ntlp::mri *m, 
		uint32 sii) : sid(id), routing_info(m), sii_handle(sii), msg(body) 
{
...
{
	return new ntlp_msg(
		get_session_id(), get_anslp_msg()->copy(),
...
ntlp_msg *ntlp_msg::copy_for_forwarding(ntlp::mri *new_mri) const 
{
	...
	return new ntlp_msg(
		get_session_id(), get_anslp_msg()->copy(), new_mri, 0);
}


/**
 * Creates an error ANSLP RESPONSE with the given severity and response code.
 */
ntlp_msg *ntlp_msg::create_response(uint8 severity, uint8 response_code) const 
{

	anslp_msg *msg = get_anslp_msg();
...
/**
 * Creates an error ANSLP RESPONSE based on the given request_error.
 */
...
/**
 * Create a ANSLP RESPONSE indicating success.
 */
		...
		uint32 session_lifetime) const 
{
	...
	anslp_response *resp
		= dynamic_cast<anslp_response *>(msg->get_anslp_msg());
	assert( resp != NULL );
 ...
 */
bool anslp::msg::is_greater_than(uint32 s1, uint32 s2) 
{
...
std::ostream &anslp::msg::operator<<(std::ostream &out, const ntlp_msg &s) 
{
	out << "[ntlp_msg: ";

	out << std::endl << "sii = " << s.get_sii_handle() << std::endl;

	if ( s.get_anslp_msg() != NULL )
		out << *(s.get_anslp_msg());
	else
		out << "[no body]";
