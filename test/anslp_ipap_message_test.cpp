...
void Anslp_IpAp_Message_Test::testAddTemplate()
{
	...
	{ 
		...
		(mes->ip_message).add_field(templatedataid, 0, IPAP_FT_IDAUCTION, 65535);
		(mes->ip_message).add_field(templatedataid, 0, IPAP_FT_STARTSECONDS, 4);
		(mes->ip_message).add_field(templatedataid, 0, IPAP_FT_ENDSECONDS, 4);
		(mes->ip_message).add_field(templatedataid, 0, IPAP_FT_AUCTIONINGALGORITHMNAME, 65535);
...
void Anslp_IpAp_Message_Test::testExceptionAddTemplate()
{
	...
	CPPUNIT_ASSERT_THROW( (mes->ip_message).add_field(templateAuctionid, 0, IPAP_FT_STARTSECONDS, 4), 
						  ipap_bad_argument);

	CPPUNIT_ASSERT_THROW( (mes->ip_message).add_field(templateBidid, 0, IPAP_FT_STARTSECONDS, 4), 
						  ipap_bad_argument);

	CPPUNIT_ASSERT_THROW( (mes->ip_message).add_field(templateAllocationid, 0, IPAP_FT_STARTSECONDS, 4), 
						  ipap_bad_argument);
			
	// Verifies that only add a valid field in the collection
	CPPUNIT_ASSERT_THROW( (mes->ip_message).add_field(templateAuctionid, 0, 3000, 4),
						  ipap_bad_argument);
	...
	(mes->ip_message).add_field(templateAuctionid, 0, IPAP_FT_STARTSECONDS, 4);
	(mes->ip_message).add_field(templateAuctionid, 0, IPAP_FT_ENDSECONDS, 4);
	(mes->ip_message).add_field(templateAuctionid, 0, IPAP_FT_STARTMILLISECONDS, 8);
	CPPUNIT_ASSERT_THROW( (mes->ip_message).add_field(templateAuctionid, 0, IPAP_FT_ENDMILLISECONDS, 8),
						  ipap_bad_argument);
	...
	CPPUNIT_ASSERT_THROW( (mes->ip_message).make_template(a, 4, IPAP_SETID_ALLOCATION_TEMPLATE), 
							ipap_bad_argument);
...
#ifdef DEBUG
	log->dlog(ch, "Ending ExportImport - Offset: %d", 
					(mes->ip_message).get_offset()  );
#endif
